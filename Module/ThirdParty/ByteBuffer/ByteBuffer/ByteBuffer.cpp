...
{
    std::unique_ptr<ByteBuffer> ret = std::make_unique<ByteBuffer>((uint32_t)buf.size());
...
std::string ByteBuffer::getNullTerminatedString() const
{
    std::string s;
    while (true)
    {
        char c = readUnsafe<char>();
        if (c == 0x00)
        {
            return s;
        }
...
std::wstring ByteBuffer::getNullTerminatedWideString() const
{
    std::wstring s;
    while (true)
    {
        char16_t c = read<char16_t>();
        if (c == 0x0000)
        {
            return s;
        }
...
std::wstring ByteBuffer::getNullTerminatedWideStringAsAscii() const
{
    std::wstring s;
    while (true)
    {
...
void ByteBuffer::putNullTerminatedString(const char* str)
{
...
void ByteBuffer::putNullTerminatedWideString(const wchar_t* str)
{
    for (int i = 0; true; i++)
    {
...
std::vector<uint8_t> ByteBuffer::getBuf()
{
    return buf;
}
