...
public:
	...
	std::string getNullTerminatedString() const;
	std::wstring getNullTerminatedWideString() const;
	std::wstring getNullTerminatedWideStringAsAscii() const;
	...
	void putNullTerminatedString(const char* str);
	void putNullTerminatedWideString(const wchar_t* str);
	...
	template<typename T> T readUnsafe() const {
		T data = readUnsafe<T>(rpos);
		rpos += sizeof(T);
		return data;
	}
	...
	template<typename T> T readUnsafe(uint32_t index) const {
		return *((T*) &buf[index]);
	}
